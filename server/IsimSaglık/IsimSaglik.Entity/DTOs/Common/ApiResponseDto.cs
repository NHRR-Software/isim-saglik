namespace IsimSaglik.Entity.DTOs.Common
{
    ...
    {
        public static ApiResponseDto<T> Success(T data, int status = 200)
        {
            return new ApiResponseDto<T>(status, true, data, null, null, DateTime.UtcNow);
        }

        public static ApiResponseDto<object> SuccessNoContent(int status = 200)
        {
            return new ApiResponseDto<object>(status, true, null, null, null, DateTime.UtcNow);
        }
